...
class TestDataset():
    def __init__(self, config, data_dir, setting):
        ...
        self.intrinsics = [591.0125, 590.16775, 322.525, 244.11084]
        # self.intrinsics = [608.309631, 608.506592, 325.733093, 243.776382]

        # real test
        result_pkl_list = glob.glob(os.path.join(self.data_dir, 'data', 'segmentation_results', 'test_trainedwithMask', 'results_*.pkl')) #with
        
        # camera test
        # result_pkl_list = glob.glob(os.path.join(self.data_dir, 'data', 'segmentation_results', 'CAMERA25', 'results_*.pkl')) #with
        
        # grasping test
        # result_pkl_list = glob.glob(os.path.join(self.data_dir, 'data', 'maskrcnn_results_grasping', 'results_*.pkl')) #with
    ...
    def __getitem__(self, index):
        ...
        if self.setting == 'supervisedwithReal275Mask':
            pred_data = data
            pred_mask = data['pred_masks']
        elif self.setting == 'supervisedwoReal275Mask':
