...
def plot_mAP(iou_aps, pose_aps, out_dir, iou_thres_list, degree_thres_list, shift_thres_list):
    ...
    labels = ['NOCS (S+R)', 'SPD (S+R)', 'Ours (S)', 'Ours (S+R)']
    colors = ['tab:green', 'tab:orange', 'tab:blue', 'tab:red']
    styles = ['--', '--', '-', '-']

    fig, (ax_iou, ax_degree, ax_shift) = plt.subplots(1, 3, figsize=(10.5, 3.5))
    # IoU subplot
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig(os.path.join('results_of_other_paper', 'mAP.png'), dpi=860)  #!!!!!!!
    plt.close(fig)
    return
...
def evaluate(path, logger=None):
    ...
    # iou_aps, pose_aps, iou_acc, pose_acc = compute_mAP(final_results, path, degree_thres_list, shift_thres_list,
    #                                                    iou_thres_list, iou_pose_thres=0.1, use_matches_for_pose=True)
    # # # metric
    # fw = open('{0}/eval_logs.txt'.format(path), 'a')
    ...
    # load Oursresults (just sys data for training)
    # pkl_path = os.path.join('results_of_other_paper', 'just_sys_mAP_Acc.pkl')
    # with open(pkl_path, 'rb') as f:
    #     our1_results = cPickle.load(f)
    # our1_iou_aps = our1_results['iou_aps'][-1, :]
    # our1_pose_aps = our1_results['pose_aps'][-1, :, :]

    # # load Oursresults
    # pkl_path = os.path.join('results_of_other_paper', 'mAP_Acc_supervised.pkl')
    # with open(pkl_path, 'rb') as f:
    #     our2_results = cPickle.load(f)
    # our2_iou_aps = our2_results['iou_aps'][-1, :]
    # our2_pose_aps = our2_results['pose_aps'][-1, :, :]

    # # load NOCS results
    # pkl_path = os.path.join('results_of_other_paper', 'mAP_Acc_nocs.pkl')
    # with open(pkl_path, 'rb') as f:
    ...
    # # load SPD results
    # pkl_path = os.path.join('results_of_other_paper', 'mAP_Acc_spd.pkl')
    # with open(pkl_path, 'rb') as f:
    #     spd_results = cPickle.load(f)
    # spd_iou_aps = spd_results['iou_aps'][-1, :]
    # spd_pose_aps = spd_results['pose_aps'][-1, :, :]
    # iou_aps = np.concatenate((our1_iou_aps[None, :], nocs_iou_aps[None, :], spd_iou_aps[None, :], our1_iou_aps[None, :], our2_iou_aps[None, :]), axis=0)
    # pose_aps = np.concatenate((our1_pose_aps[None, :, :], nocs_pose_aps[None, :, :], spd_pose_aps[None, :, :], our1_pose_aps[None, :, :], our2_pose_aps[None, :, :]), axis=0)

    # plot
    # plot_mAP(iou_aps, pose_aps, path, iou_thres_list, degree_thres_list, shift_thres_list)
